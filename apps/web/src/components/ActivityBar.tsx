...
export default function ActivityBar({
  ...
  const renderItem = (item: (typeof SIDEBAR_ITEMS)[number]) => {
    ...
    return (
      <div key={item.id} className="relative flex justify-center">
        {isActive && (
          <span className="absolute left-0 top-1/2 h-6 w-[3px] -translate-y-1/2 rounded-r bg-ink" />
        )}
        <button
          onClick={() => onSelect(item.id)}
          className={[
            "relative flex h-10 w-10 items-center justify-center rounded-lg transition-colors",
            isActive ? "text-ink" : "text-ink/40 hover:bg-ink/5 hover:text-ink/70",
          ].join(" ")}
          title={item.label}
  ...
  return (
    <div className="z-20 hidden w-12 shrink-0 flex-col items-stretch border-r border-ink/10 bg-paper/80 py-2 backdrop-blur sm:flex">
