  ...
  ws.onmessage = (e: MessageEvent) => {
    try {
      ...
      if (msg.type === "state") {
        ...
        // Two shapes share this type: the lightweight presence pulse
        // ({action:"read"}) and the action-lifecycle fact, which is the one
        // carrying an actionId. Route each to its own subscribers.
        if (msg.actionId) {
