...
### Intent facade (12 tools) + `npx tandem init`
- The facade **replaces** the 80-tool CRUD surface as the **default** connector manifest. Full CRUD moves behind an opt-in flag (`TANDEM_FULL_TOOLS=1`, or `tandem-mcp --full-tools`). Twelve tools on top of eighty is tool ninety-two — the default surface IS the facade.
- Shipped set (`apps/mcp-gateway/src/facade.ts`): `canvas_connect` (kept; also mints identity via `role`), `agent_register`, `context_get`, `queue_next`, `task_get`, `task_claim`, `task_progress`, `task_complete`, `task_propose`, `epic_propose`, `doc_write` (agent output container), `board_status`. This planned at ~10; `agent_register` and `epic_propose` were added during E2/E9, so **12 is the number** — it is what `manifest.test.ts` asserts and what the docs must say.
