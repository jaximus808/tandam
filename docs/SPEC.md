...
- **Superseded — the two bullets above are the ratified spec, not the current surface.** The facade has since grown to **17**: `queue_wait` (TDM-149), `task_review` (TDM-156, which *replaced* `task_approve` rather than joining it — still routed, no longer advertised), and `doc_read` (TDM-180, the read side of `doc_write`, on `GET /api/canvas/documents/{ref}/notes` from TDM-179 — it retires the hand-built raw-API read sessions used to fall back on). `FACADE_NAMES` in `apps/mcp-gateway/src/facade.ts` is the source of truth, and `manifest.test.ts` asserts against it; the root [README](../README.md#mcp) and [apps/mcp-gateway/README.md](../apps/mcp-gateway/README.md#tools) carry the live list.
